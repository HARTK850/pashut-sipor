class StoryAI {
  ...
  async saveApiKey() {
    ...
    this.showApiKeyStatus(false, "בודק את תקינות המפתח...")
    ...
    try {
      ...
      this.showApiKeyStatus(false, `מפתח API לא תקין: ${error.message}`)
    }
  }
  ...
  async generateStoryText(topic) {
    ...
    כתב רק את הסיפור, ללא הסברים נוספים.`
    ...
    try {
      ...
      if (candidate.finishReason === "SAFETY") {
        throw new Error("הנושא שבחרת לא מתאים ליצירת סיפור. אנא נסה נושא אחר.")
      }

      const storyText = candidate.content?.parts?.[0]?.text

      if (!storyText) {
        throw new Error("לא הצלחתי ליצור את הסיפור. אנא נסה שוב.")
      }
  ...
  async generateAudio(text) {
    try {
      // Since Google's TTS API for Gemini might not be available yet,
      // we'll use Web Speech API as a fallback
  ...
  generateWebSpeechAudio(text) {
    return new Promise((resolve) => {
      try {
        ...
        const mediaRecorder = this.createAudioRecorder(utterance)

        if (mediaRecorder) {
          resolve(mediaRecorder)
        } else {
          // If recording fails, just play without saving
  ...
  createAudioRecorder(utterance) {
    try {
      // This is a simplified approach - in reality, recording speech synthesis
      // is complex and might not work in all browsers
      speechSynthesis.speak(utterance)
      return null // Return null to indicate no downloadable audio
    } catch (error) {
  ...
  displayStory(topic, storyText, audioUrl) {
    ...
    if (audioUrl) {
      ...
      // Show text-to-speech option instead
      audioPlayer.innerHTML = `
            <h3>האזן לסיפור:</h3>
            <button id="playTextToSpeech" class="btn btn-primary">השמע באמצעות דיבור ממוחשב</button>
            <p style="font-size: 14px; color: #718096; margin-top: 10px;">
                הדפדפן ישתמש בדיבור ממוחשב כדי להקריא את הסיפור
            </p>
        `
      audioPlayer.style.display = "block"

      // Add event listener for text-to-speech
      document.getElementById("playTextToSpeech").addEventListener("click", () => {
        this.playTextToSpeech(storyText)
      })
    }
  ...
  playTextToSpeech(text) {
    ...
    const playButton = document.getElementById("playTextToSpeech")
    const originalText = playButton.textContent

    utterance.onstart = () => {
      playButton.textContent = "מקריא..."
  ...
  loadHistory() {
    ...
    historyList.innerHTML = this.stories
      .map(
        (story) => `
